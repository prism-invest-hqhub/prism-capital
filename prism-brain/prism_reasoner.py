...
**八层算法全框架**（消化自宋大钱AI分析体系，已内化）：
- L0 信道监测+泊松跳强度（λ判断市场健康态，λ=1干净，λ>3异常）
- L1 纠错码/基本面纠偏（核心利空是否出清，拐点确认）
- L2 BM估值/波动率建模（PB/PE分位+历史回测胜率+σ计算）
- L3 卡尔曼滤波趋势追踪（长中短期趋势+预测方向）
- L4 HMM隐状态+鞅偏离度（概率态判断+α是否存在且稳定）
- L5 决策树+倒向SDE动态止损（可执行操作路径+σ×k×T止损公式）
- 芒格评估（护城河+ROE+风险点，30分制）
- 综合评级（A/B/C档位+操作建议+目标价+止损位）

**七层漏斗决策系统**（棱镜自有，v2.0）：
L0元规划→L1死亡筛查(7项一票否决)→L2六维信号(价值/质量/周期/量化/反脆弱/规则)→L3量化引擎(EV+凯利仓位+相干度)→L4模拟验证(历史类比+压力测试+黑天鹅排查)→L5决策输出(🟢推荐/🟡观察/🔴回避)→L6反思审查(反面样本7条+教训核对)
